...
let channel_point;
// module.exports = (app) => {
module.exports = (app, lightning, db, config, walletUnlocker, lnServicesData, mySocketsEvents) => {
	...
	app.get('/api/lnd/connect', function(req, res) {
		res.status(200);
		res.json({});
	});

	app.post('/api/mobile/error', function(req, res) {
		console.log(JSON.stringify(req.body));
		res.json({msg: OK});
	});

	app.get('/api/lnd/auth', function(req, res) {
	...
	let recreateLnServices = (callback, cs) => {
		...
		let lnServices = require("../services/lnd/lightning")(lnServicesData.lndProto, lnServicesData.lndHost, lnServicesData.lndCertPath, lnServicesData.macaroonPath);
	...
	app.post('/api/lnd/connect', function(req, res) {
		let args = {
	...
	app.post('/api/lnd/wallet', function(req, res) {
		let mnemonicPhrase;
		walletUnlocker.genSeed({}, function (genSeedErr, genSeedResponse) {
			if (genSeedErr) {
				...
				walletUnlocker.initWallet(walletArgs, function (initWalletErr, initWalletResponse) {
					if (initWalletErr) {
						console.log("initWallet Error:", initWalletErr.message);
						console.log("initWallet Error:", Object.keys(initWalletErr.message));
						return checkHealth().then(healthResponse => {
							if (healthResponse.connectedToLnd) {
						...
						const waitUntilFileExists = (seconds) => {
						  console.log(`Waiting for admin.macaroon to be created. Seconds passed: ${seconds}`);
						  setTimeout(() => {
		...
		app.get('/api/static/profiles', function(req, res) {
			let options = {
				method: 'GET',
				uri: 'http://static.shock.network:8020/profiles',
			};
			return rp(options).then(function (profiles) {
				// [req.method][req.path]
		...
		app.put('/api/static/profile', function(req, res) {
			lightning.getInfo({}, function (err, response) {
				if (err) {
	...
	// openchannel
	app.post("/api/lnd/openchannel", function (req, res) {
		if (req.limituser) {
			return checkHealth().then(healthResponse => {
				if (healthResponse.connectedToLnd) {
	...
	// closechannel
	app.post("/api/lnd/closechannel", function (req, res) {
		if (req.limituser) {
			return checkHealth().then(healthResponse => {
				if (healthResponse.connectedToLnd) {
					// return res.sendStatus(403); // forbidden
